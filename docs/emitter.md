...
> Location within the framework `Carnor-agent-framework/emitter`.
...
```ts
import { Emitter, EventMeta } from "Carnor-agent-framework/emitter/emitter";
...
```ts
import { Emitter, EventMeta, Callback } from "Carnor-agent-framework/emitter/emitter";
...
await emitter.emit("update", { id: 123, data: "Hello Carnor!" });
```
...
> [!TIP]
>
> The more complex agentic example can be found [here](/examples/agents/Carnor.ts).
