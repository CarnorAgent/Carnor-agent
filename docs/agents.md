...
## Implementation in Carnor Agent Framework
...
Our Carnor Agent is based on the `ReAct` ([Reason and Act](https://arxiv.org/abs/2210.03629)) approach.
...
const agent = new CarnorAgent({
  llm: new OllamaChatLLM(), // for more explore 'Carnor-agent-framework/adapters'
  memory: new UnconstrainedMemory(), // for more explore 'Carnor-agent-framework/memory'
  tools: [new OpenMeteoTool()], // for more explore 'Carnor-agent-framework/tools'
});
...
Please refer to the [following example](/examples/agents/Carnor_advanced.ts) to see how to modify them.
