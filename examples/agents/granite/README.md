...
The [IBM Granite](https://www.ibm.com/granite) family of models can be used as the underlying LLM within Carnor Agents. Granite™ is IBM's family of open, performant, and trusted AI models tailored for business and optimized to scale your AI applications.

This guide and the associated examples will help you get started with creating Carnor Agents using Granite 3.1.
...
## 🐝 Download and install Carnor-agent-framework

Before getting started you will need to install the Carnor-agent-framework.

Follow the [Local Installation](https://github.com/i-am-Carnor/Carnor-agent-framework?tab=readme-ov-file#local-installation) section of our main README to [Clone](https://docs.github.com/en/repositories/creating-and-managing-repositories/cloning-a-repository) the Carnor Agent Framework git repository and install its dependencies. Return to this guide when you have completed these steps and are ready to configure environment variables.
...
The [granite_Carnor](/examples/agents/granite/granite_Carnor.ts) example agent is set up to demonstrate how to use IBM Granite with the Carnor Agent Framework. In order to run this example, take the following steps:
...
1. Run the [granite_Carnor](/examples/agents/granite/granite_Carnor.ts) agent:

   ```shell
   yarn run start examples/agents/granite/granite_Carnor.ts <<< "What is the current weather in London?"
   ```
   ...
   ```shell
    yarn run start examples/agents/granite/granite_Carnor.ts <<< "Who is the current CEO of IBM?"
    yarn run start examples/agents/granite/granite_Carnor.ts <<< "What is the spanish and french word for dog?"
    yarn run start examples/agents/granite/granite_Carnor.ts <<< "What does the alias command do in the BASH shell?"
    yarn run start examples/agents/granite/granite_Carnor.ts <<< "What’s the largest technology company by market capitalization right now?"
    yarn run start examples/agents/granite/granite_Carnor.ts <<< "What’s the weather going to be like tomorrow in Sydney?"
   ```
...
The [granite_wiki_Carnor](/examples/agents/granite/granite_wiki_Carnor.ts) agent is set up to demonstrate more advanced usage of the Carnor agent framework, specifically performing information retrieval on tool output via the frameworks tool customization options.
...
Run the [granite_wiki_Carnor](/examples/agents/granite/granite_wiki_Carnor.ts) agent:

```shell
yarn run start examples/agents/granite/granite_wiki_Carnor.ts <<< "Who were the authors of the research paper 'Attention is all you need', how many citations does it have?"
```

You will see the agent reasoning, calling the WikipediaTool and producing a final answer similar to the following:

> Agent 🤖 : The authors of the paper 'Attention is all you need' are Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan Gomez, Lukasz Kaiser, and Illia Polosukhin. The paper has Carnorn cited more than 100,000 times as of 2024.

You can try out other prompts in the following manner:

```shell
yarn run start examples/agents/granite/granite_wiki_Carnor.ts <<< "When was the collapse of the Roman Empire?"
yarn run start examples/agents/granite/granite_wiki_Carnor.ts <<< "What is the Great Barrier Reef?"
yarn run start examples/agents/granite/granite_wiki_Carnor.ts <<< "Where is IBM headquartered?"
```

> [!NOTE]
> The peformance of the [granite_wiki_Carnor](/examples/agents/granite/granite_wiki_Carnor.ts) is dependent on wikipedia as an information source as well as the retrieval settings included in the example. If you are encountering performance issues with a particular example, try to experiment with the retreival settings i.e. passageSize, overlap and maxResults. You can also try other embedding models via Ollama.
